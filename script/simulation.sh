...
function NGS_simulation()
{
    ...
    # wgsim -e $4 -d $3 -s 35 -1 $2 -2 $2 -r 0 -R 0 -X 0 $6 ${5}_r1.fq ${5}_r2.fq

    # formula: coverage = (read count * read length ) / total genome size
    # For PE sequencing, read count doubles    
    wgsim -1 $2 -2 $2 -d $3 -N 2400 -r 0 -R 0 -X 0 -e 0 $6 ${5}_r1.fq ${5}_r2.fq
...
# 1: reference fasta
# 2: prefix
# 3: real pacbio clr reads for homo sapien
...
{    
    ...
    pbsim --prefix ${2}_pb --depth 30 --hmm_model /home/eric/pbsim2/data/P6C4.model $1
...
function nanopore_simulation()
{
    ...
    # /home/eric/DeepSimulator/deep_simulator.sh -i $1 -c 20 -B 2 -K 30 -o ${2}_ont
    pbsim --prefix ${2}_ont --depth 30 --hmm_model /home/eric/pbsim2/data/R103.model $1
    # cat paternal and maternal by user
}
# NGS_simulation 30 150 200 0 bfb ./bfb.fa
# NGS_simulation 30 150 200 0 normal ./normal.fa

# pacbio_simulation ./bfb.fa bfb4

nanopore_simulation ./bfb1.fa bfb1
...
# 7: maternal fasta
# 8: molecule_len (50)
# tenx_simulation 30 150 350 0 10x ./bfb.fa ./bfb.fa 50